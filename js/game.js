...
function playSfx(kind) {
    ...
    } else if (kind === 'menuTap') {
        playTone(520, 0.06, 'triangle', audioSystem.sfxGain, 0.045);
        playTone(700, 0.08, 'sine', audioSystem.sfxGain, 0.03);
    }
}

function setMusicMode(mode) {
    if (audioSystem.musicMode === mode) {
        return;
    }
...
function playMusicStep() {
    ...
    const isMenuMode = audioSystem.musicMode === 'menu';
    const isGameMode = audioSystem.musicMode === 'game';
    const normalScale = [220, 262, 294, 330, 349, 392, 440];
    const bossScale = [130, 146, 174, 196, 220, 246, 261];
    const menuScale = [196, 247, 294, 330, 392, 440, 494];
    const isBossTime = bosses.length > 0;

    const scale = isMenuMode ? menuScale : (isBossTime ? bossScale : normalScale);
    const interval = isMenuMode
        ? 420
        : Math.max(190, 360 - phase * 16 - (isBossTime ? 80 : 0));
    ...
    audioSystem.noteIntervalId = window.setInterval(() => {
        const modeNow = audioSystem.musicMode;
        if (modeNow === 'game' && !gameRunning) {
            return;
        }

        if (modeNow === 'menu' && gameRunning) {
            return;
        }

        const idx = (audioSystem.sequenceStep + phase) % scale.length;
        if (modeNow === 'menu') {
            const menuFreq = scale[idx];
            playTone(menuFreq, 0.22, 'sine', audioSystem.musicGain, 0.04);
            if (audioSystem.sequenceStep % 3 === 0) {
                playTone(menuFreq * 0.5, 0.18, 'triangle', audioSystem.musicGain, 0.025);
            }
        } else {
            const bossNow = bosses.length > 0;
            const activeScale = bossNow ? bossScale : normalScale;
            const activeIdx = (audioSystem.sequenceStep + phase) % activeScale.length;
            const baseFreq = activeScale[activeIdx] * (bossNow ? 1 : 1 + (phase % 4) * 0.06);
            const noteType = bossNow ? 'sawtooth' : 'triangle';

            playTone(baseFreq, 0.16, noteType, audioSystem.musicGain, bossNow ? 0.06 : 0.045);
            if (!bossNow && audioSystem.sequenceStep % 2 === 0) {
...
function initGame() {
    ...
    window.addEventListener('orientationchange', updateControlDensity);
}
